    ...
    fonts = new Array(1);
    	...
    	merges = [];
			...
			if (options.page) {
				...
				if (options.page.fit_to_height) fit_to_height = options.page.fit_to_height;
			}
			...
			s += '<pageSetup paperSize="' + paper_size + '" orientation="' + orientation + '" scale="' + page_scale + '" ' + (fit_to_height != '' ? ' fitToHeight="' + fit_to_height + '" ' : '') + '/>'
			if (worksheet.table) { 
		...
		while (--i) { 
			...
			styles[i] = ['<xf xfId="0" fillId="0" borderId="', 
				...
				style.formatCode,
				'" ',
				(style.hAlign || style.vAlign || style.wrapText || style.textRotation? 'applyAlignment="1" ' : ' '),
				(style.formatCode > 0 ? 'applyNumberFormat="1" ' : ' '),
				(borderIndex > 0 ? 'applyBorder="1" ' : ' '),
				(fontIndex > 0 ? 'applyFont="1" ' : ' '),
				'>'
			];
			if (style.hAlign || style.vAlign || style.textRotation || style.wrapText) {
